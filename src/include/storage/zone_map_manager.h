...
class ZoneMapManager {
 ...
 private:
  ...
  type::Value GetValueAsOriginal(type::Value val, type::Value actual_type) {
    ...
    type::Type::TypeId type_id = StringToTypeId(type_str);
  ...
  bool checkEqual(type::Value predicateVal, ColumnStatistics *stats) {
    return ((stats->min).CompareLessThanEquals(predicateVal)) == type::CmpBool::CMP_TRUE &&
           ((stats->max).CompareGreaterThanEquals(predicateVal) == type::CmpBool::CMP_TRUE);
  }

  bool checkLessThan(type::Value predicateVal, ColumnStatistics *stats) {
    return (predicateVal.CompareGreaterThan(stats->min) == type::CmpBool::CMP_TRUE);
  }

  bool checkLessThanEquals(type::Value predicateVal, ColumnStatistics *stats) {
    return (predicateVal.CompareGreaterThanEquals(stats->min) == type::CmpBool::CMP_TRUE);
  }

  bool checkGreaterThan(type::Value predicateVal, ColumnStatistics *stats) {
    return (predicateVal.CompareLessThan(stats->max) == type::CmpBool::CMP_TRUE);
  }

  bool checkGreaterThanEquals(type::Value predicateVal,
                              ColumnStatistics *stats) {
    return (predicateVal.CompareLessThanEquals(stats->max) == type::CmpBool::CMP_TRUE);
  }
