...
namespace logging {
  ...
  void CheckpointManager::Running() {
    ...
    while (1) {
      ...
      if (count == checkpoint_interval_ || is_init == true) {
        ...
        cid_t begin_cid = epoch_manager.EnterEpoch(0,TimestampType::SNAPSHOT_READ);
        ...
        auto catalog_manager = storage::StorageManager::GetInstance();
        auto database_count = catalog_manager->GetDatabaseCount();

        database_structures.resize(database_count);

        for (oid_t database_idx = 0; database_idx < database_count; ++database_idx) {
          auto database = catalog_manager->GetDatabaseWithOffset(database_idx);
  ...
  void CheckpointManager::RecoverCheckpointThread(const size_t &thread_id, const size_t &epoch_id, const std::vector<std::vector<size_t>> &database_structures, FileHandle ***file_handles) {
    ...
    concurrency::TransactionContext* current_txn = concurrency::TransactionManagerFactory::GetInstance().BeginTransaction(thread_id, IsolationLevelType::SERIALIZABLE);
