...
bool ReorderedPhyLogLogger::InstallTupleRecord(LogRecordType type, storage::Tuple *tuple, storage::DataTable *table, cid_t cur_cid) {
  ...
  auto pindex = table->GetIndexWithOid(0);
  ...
  if (old_cid < cur_cid) {
    ...
    if (type != LogRecordType::TUPLE_DELETE) {
      ContainerTuple<storage::TileGroup> allocated_location(tg.get(), overwrite_location.offset);
  ...
  for (size_t tg_idx = logger_id_; tg_idx < tg_count; tg_idx += logger_count) {
    ...
    for (oid_t tuple_offset = 0; tuple_offset < active_tuple_count; ++tuple_offset) {
     ...
     /* if (concurrency::TransactionManagerFactory::IsRB() == false &&
        ...
        res = table->InsertInIndexes(&container_tuple, ItemPointer(tg_id, tuple_offset), txn, nullptr);
      //}
