...
#include "logging/reordered_physical_logger.h"
...
void ReorderedPhysicalLogger::GetSortedLogFileIdList(const size_t checkpoint_eid, const size_t persist_eid) {
  ...
  size_t file_epoch_count = (size_t)(new_file_interval_ / 10);
...
txn_id_t ReorderedPhysicalLogger::LockTuple(storage::TileGroupHeader *tg_header, oid_t tuple_offset) {
  txn_id_t txnid_logger = (INITIAL_TXN_ID + this->logger_id_);
  while (true) {
...
void ReorderedPhysicalLogger::UnlockTuple(storage::TileGroupHeader *tg_header, oid_t tuple_offset, txn_id_t new_txn_id) {
  ...
  tg_header->SetAtomicTransactionId(tuple_offset, (txn_id_t) (INITIAL_TXN_ID + this->logger_id_), new_txn_id);
}
    ...
    case LogRecordType::TUPLE_UPDATE: {
        ...
        )
      {
        ...
        UnlockTuple(new_tg_header, new_tuple_offset,
                    (type == LogRecordType::TUPLE_UPDATE) ? INITIAL_TXN_ID : INVALID_TXN_ID);
...
bool ReorderedPhysicalLogger::ReplayLogFile(const size_t thread_id, FileHandle &file_handle, size_t checkpoint_eid, size_t persist_eid) {
  ...
  while (true) {
    ...
    concurrency::TransactionContext* current_txn = nullptr;
    switch (record_type) {
      case LogRecordType::EPOCH_BEGIN: {
        ...
        if (current_eid >= checkpoint_eid && current_eid <= persist_eid) {
          ...
          current_txn = concurrency::TransactionManagerFactory::GetInstance().BeginTransaction(thread_id, IsolationLevelType::SERIALIZABLE);
        }
        ...
        if (current_eid >= checkpoint_eid && current_eid <= persist_eid) {
          // TODO: Do we really need this -- Jiexi
          concurrency::TransactionManagerFactory::GetInstance().EndTransaction(current_txn);
        }
...
void ReorderedPhysicalLogger::RunIndexRebuildThread(const size_t logger_count) {
  ...
  for (oid_t db_idx = 0; db_idx < db_count; db_idx ++) {
    auto database = manager->GetDatabaseWithOffset(db_idx);
...
void ReorderedPhysicalLogger::RebuildIndexForTable(const size_t logger_count, storage::DataTable *table) {
  ...
  for (size_t tg_idx = logger_id_; tg_idx < tg_count; tg_idx += logger_count) {
    ...
    oid_t tg_capacity = DEFAULT_TUPLES_PER_TILEGROUP;
    for (oid_t tuple_offset = 0; tuple_offset < tg_capacity; ++tuple_offset) {
      ...
      if (tg_header->GetTransactionId(tuple_offset) == INITIAL_TXN_ID) {
        ...
        ContainerTuple<storage::TileGroup> container_tuple(tg, tuple_offset);
        ...
        auto res = table->InsertInIndexes(&container_tuple, ItemPointer(tg_id, tuple_offset), txn, &itemptr);
        concurrency::TransactionManagerFactory::GetInstance().EndTransaction(txn);
        if (res == false) {
    ...
    // Pull log records from workers per epoch buffer
    {
      ...
      for (auto &worker_entry : worker_map_) {
        ...
        for (size_t epoch_id = last_persist_eid + 1; epoch_id < worker_current_eid; ++epoch_id) {
          ...
          size_t epoch_idx = epoch_id % 40960;
...
void ReorderedPhysicalLogger::PersistEpochBegin(FileHandle &file_handle, const size_t epoch_id) {
  ...
  logger_output_buffer_.WriteEnumInSingleByte(static_cast<int>(LogRecordType::EPOCH_BEGIN));
...
void ReorderedPhysicalLogger::PersistEpochEnd(FileHandle &file_handle, const size_t epoch_id) {
  ...
  logger_output_buffer_.WriteEnumInSingleByte(static_cast<int>(LogRecordType::EPOCH_END));
  logger_output_buffer_.WriteLong((uint64_t) epoch_id);
