...
type::Value Tile::GetValue(const oid_t tuple_offset, const oid_t column_id) {
  ...
  const type::Type::Type::TypeId column_type = schema.GetType(column_id);
...
type::Value Tile::GetValueFast(const oid_t tuple_offset,
                                 ...
                                 const type::Type::Type::TypeId column_type,
...
bool Tile::SerializeHeaderTo(SerializeOutput &output) {
  ...
  for (oid_t column_itr = 0; column_itr < column_count; ++column_itr) {
    type::Type::Type::TypeId type = schema.GetType(column_itr);
...
void Tile::DeserializeTuplesFrom(SerializeInput &input,
  ...
  // failure
  type::Type::Type::TypeId types[column_count];
  std::vector<std::string> names;

  // Skip the column types
  for (oid_t column_itr = 0; column_itr < column_count; ++column_itr) {
    types[column_itr] = (type::Type::Type::TypeId)input.ReadEnumInSingleByte();
  }
